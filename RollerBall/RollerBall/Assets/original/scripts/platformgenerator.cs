...
public class platformgenerator : MonoBehaviour {
    ...
    private void SpawnTile(int prefabIndex = -1)
    {
        ...
        { 
            if ((playerTransform.position.z > 0)&& playerTransform.position.z < 80)
            {
                newtile();
            }
        }
        ...
        {
            if ((playerTransform.position.z > 256) && playerTransform.position.z < 650)
            {
                newtile();
            }
        }

        if (this.gameObject.name == "Plane4")

        {
            if ((playerTransform.position.z > 650) && playerTransform.position.z < 1100)
            {
                newtile();
            }
        }
